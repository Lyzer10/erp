...
function intersectOneDim(a0, a1, b0, b1, updateDimIdx, useMTV, outIntersectRect, clamp) {
  var d0 = mathAbs$3(a1 - b0);
  var d1 = mathAbs$3(b1 - a0);
  var d01min = mathMin$4(d0, d1);
  ...
  if (a1 < b0 || b1 < a0) {
    ...
    if (outIntersectRect) {
      outIntersectRect[updateDim] = mathMax$4(a0, b0);
      outIntersectRect[wh] = mathMin$4(a1, b1) - outIntersectRect[updateDim];
    }
    if (useMTV) {
      if (d01min < _lenMinMax[0] || _intersectCtx$1.useDir) {
        _lenMinMax[0] = mathMin$4(d01min, _lenMinMax[0]);
  ...
  var _ctx = {
    ...
    reset: function(opt, useMTV) {
      ...
      if (opt && opt.touchThreshold != null) {
        _ctx.touchThreshold = mathMax$4(0, opt.touchThreshold);
      }
...
function fromCubic(x0, y0, x1, y1, x2, y2, x3, y3, min3, max3) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    min3[0] = mathMin$3(x, min3[0]);
    max3[0] = mathMax$3(x, max3[0]);
  }
  n = cubicExtrema$1(y0, y1, y2, y3, yDim);
  for (var i = 0; i < n; i++) {
    var y = cubicAt$1(y0, y1, y2, y3, yDim[i]);
    min3[1] = mathMin$3(y, min3[1]);
    max3[1] = mathMax$3(y, max3[1]);
  }
...
function fromQuadratic(x0, y0, x1, y1, x2, y2, min3, max3) {
  ...
  min3[0] = mathMin$3(x0, x2, x);
  min3[1] = mathMin$3(y0, y2, y);
  max3[0] = mathMax$3(x0, x2, x);
  max3[1] = mathMax$3(y0, y2, y);
}
...
function modPI2(radian) {
  var n = Math.round(radian / PI$3 * 1e8) / 1e8;
  return n % 2 * PI$3;
}
...
var OrientedBoundingRect = (function() {
  ...
  OrientedBoundingRect2.prototype._getProjMinMaxOnAxis = function(dim, corners, out2) {
    ...
    for (var i = 1; i < corners.length; i++) {
      ...
      min3 = mathMin(proj_1, min3);
      max3 = mathMax(proj_1, max3);
    }
...
function brushIncremental(ctx, el, scope) {
  ...
  for (i = el.getCursor(), len2 = displayables.length; i < len2; i++) {
    ...
    brush$1(ctx, displayable, innerScope);
...
function convertPolyShape(shape, attrs, mul2) {
  ...
  for (var i = 0; i < points2.length; i++) {
    strArr.push(round(points2[i][0] * mul2) / mul2);
    strArr.push(round(points2[i][1] * mul2) / mul2);
  }
...
function setShadow(el, attrs, scope) {
  ...
  if (hasShadow(style)) {
    ...
    if (!shadowId) {
      ...
      var _a = normalizeColor(style.shadowColor), opacity = _a.opacity, color2 = _a.color;
...
function setGradient(style, attrs, target, scope) {
  ...
  for (var i = 0, len2 = colors.length; i < len2; ++i) {
    ...
    var _a = normalizeColor(stopColor), color2 = _a.color, opacity = _a.opacity;
    var stopsAttrs = {
      "offset": offset
    };
...
function createBackgroundVNode(width, height, backgroundColor, scope) {
  ...
  if (backgroundColor && backgroundColor !== "none") {
    ...
    if (isGradient(backgroundColor)) {
      ...
      var _a = normalizeColor(backgroundColor), color2 = _a.color, opacity = _a.opacity;
...
export {
  ...
  subPixelOptimizeRect as V,
  clone$3 as W,
  CompoundPath as X,
  Group as Y,
  ZRImage as Z,
  IncrementalDisplayable as _,
  isObject as a,
  boundingRectCalculateTransform as a$,
  OrientedBoundingRect as a0,
  Point as a1,
  RadialGradient as a2,
  ZRText as a3,
  normalizeCssArray as a4,
  retrieve3 as a5,
  trim as a6,
  merge as a7,
  mixin as a8,
  ...
  lerp$1 as aB,
  PathProxy as aC,
  quadraticProjectPoint as aD,
  cubicProjectPoint as aE,
  normalizeRadian as aF,
  SVGPainter as aG,
  CanvasPainter as aH,
  cubicRootAt as aI,
  cubicAt as aJ,
  ...
  rotate as aL,
  retrieve as aM,
  normalizeArcAngles as aN,
  find as aO,
  isMiddleOrRightButtonOnMouseUpDown as aP,
  stop as aQ,
  parseXML as aR,
  parseSVG as aS,
  makeViewBoxTransform as aT,
  transformableCreate as aU,
  transformableGetLocalTransform as aV,
  copy$1 as aW,
  boundingRectCopy as aX,
  boundingRectApplyTransform as aY,
  boundingRectContain as aZ,
  boundingRectCreate as a_,
  curry as aa,
  modifyHSL as ab,
  isTypedArray as ac,
  filter as ad,
  setAsPrimitive as ae,
  concatArray as af,
  noop as ag,
  Eventful as ah,
  calculateTextPosition as ai,
  WeakMap as aj,
  LRU as ak,
  platformApi as al,
  brushSingle as am,
  setPlatformAPI as an,
  init as ao,
  sort as ap,
  registerSSRDataGetter as aq,
  registerPainter as ar,
  slice as as,
  eqNaN as at,
  parsePercent as au,
  contain as av,
  ...
  len as bf,
  scaleAndAdd as bg,
  quadraticSubdivide as bh,
  quadraticAt as bi,
  distSquare as bj,
  clone$1 as bk,
  quadraticDerivativeAt as bl,
  containStroke$4 as bm,
  containStroke$2 as bn,
  lift as bo,
  TRANSFORMABLE_PROPS as bp,
  cloneValue as bq,
  isDom as br,
  addEventListener as bs,
  normalizeEvent as bt,
  transformLocalCoord as bu,
  transformLocalCoordClear as bv,
  isRegExp as bw,
  isCombineMorphing as bx,
  morphPath as by,
  combineMorph as bz,
  isStringSafe as c,
  isArray as d,
