...
export const resetDemoDataFn = createServerFn({ method: "POST" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
  const result = await ds.resetDemoData();
  return { isTestUser: ds.isTestUser, ...result };
});

export const getCustomersFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
...
export const getSuppliersFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
...
export const getProductsFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
...
export const getInvoicesFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
...
export const getExpensesFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
  const data = await ds.expenses.getAll();
  return { isTestUser: ds.isTestUser, data };
});

export const getStaffFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
  const data = await ds.staff.getAll();
  return { isTestUser: ds.isTestUser, data };
});

export const getPayrollFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
  const data = await ds.payroll.getAll();
  return { isTestUser: ds.isTestUser, data };
});

export const getStoresFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
  const data = await ds.stores.getAll();
  return { isTestUser: ds.isTestUser, data };
});

export const getTransfersFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
  const data = await ds.transfers.getAll();
  return { isTestUser: ds.isTestUser, data };
});

export const getLeaveRequestsFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
  const data = await ds.leaveRequests.getAll();
  return { isTestUser: ds.isTestUser, data };
});

export const getChartOfAccountsFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
  const data = await ds.chartOfAccounts.getAll();
  return { isTestUser: ds.isTestUser, data };
});

export const getPaymentVouchersFn = createServerFn({ method: "GET" }).handler(async ({ request }) => {
  const ds = await resolveDataSource(request);
