...
export function Topbar({ onMenuClick, onToggleSidebar }: Props) {
  ...
  return (
    <header className="glass-topbar sticky top-0 z-30 flex h-16 items-center gap-2 px-3 lg:px-6">
      ...
      <button
        ...
        className="hidden rounded-lg p-2 text-muted-foreground hover:bg-white/60 hover:text-foreground lg:inline-flex"
      ...
      {/* Crumb fallback for small screens */}
      <span className="ml-2 truncate text-xs font-medium capitalize text-muted-foreground md:hidden">
        {pathname === "/" ? "Dashboard" : pathname.split("/").filter(Boolean).join(" / ").replace(/-/g, " ")}
      </span>
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <Link
          to="/pos"
          className="group relative inline-flex items-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r from-emerald-500 to-blue-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-emerald-500/30 transition hover:shadow-emerald-500/50"
        >
        ...
        <button className="relative rounded-xl border border-white/60 bg-white/50 p-2 backdrop-blur-md transition hover:bg-white/70" aria-label="Notifications">
