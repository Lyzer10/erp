...
function InvoicesPage() {
  const cols = [
    ...
    { key: "due", header: "Due" },
    {
      ...
      render: (r: typeof invoices[0]) => <span className="font-semibold">{currency(r.amount)}</span>,
    },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Invoices"
        description="Create and manage invoices, proformas, credit notes, and cash sales."
        actions={
          <>
            <button className="inline-flex items-center gap-2 rounded-lg border border-white/60 bg-white/60 px-3 py-2 text-sm font-medium backdrop-blur hover:bg-white/80">
              <FileText className="h-4 w-4" /> Merge Invoices
            </button>
            <Link
